...
Mastra: Parse → Extract clauses → Chunk by type
    ↓
Quadrant: Embed clauses → Store → Retrieve similar standards
    ↓
Claude: Analyse risk → Draft explanations → Generate counter-clauses
    ↓
...
Quick Start: Analyse a Contract
...
│   │   ├── api/
│   │   │   ├── analyze.ts            (POST /analyse endpoint)
...
typescript
// Every output is validated before being shown to the user
...
2. Qdrant is essential — Comparing clauses requires a semantic search. Keyword matching fails. Our RAG pipeline proves a deep understanding of vector retrieval.

3. Mastra workflows are sophisticated — Parse → Embed → Retrieve → Analyse → Draft → Validate is a multi-step orchestration, not a simple chain.
